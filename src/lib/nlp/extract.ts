 ...
 * 
 * Uses the `compromise` NLP library to identify people, locations, and topics 
 * in journal entries. All extraction occurs strictly client-side to preserve privacy.
 * 
 * Flow:
 * - Sanitizes rich-text input by stripping HTML tags.
 * - Employs linguistic pattern matching to distinguish names from general nouns.
 * - Interfaces with the blacklist manager to filter out unwanted suggestions.
 * 
 * @module src/lib/nlp/extract.ts
 */
